    ...
    for (const filename of potentialProducts) {
        try {
            ...
            if (response.ok) {
                ...
                // Special debugging for the new admin product
                if (filename === 'new-admin-product.md') {
                    console.log('=== DEBUGGING NEW ADMIN PRODUCT ===');
                    console.log('File content preview:', text.substring(0, 200));
                    console.log('Has frontmatter:', text.includes('---'));
                ...
                if (text.includes('---') && text.includes('title:')) {
                    ...
                    if (filename === 'new-admin-product.md') {
                        ...
                        console.log('Title exists:', !!product.title);
                        console.log('Title value:', product.title);
                        console.log('Draft status:', product.draft);
                        console.log('Will be added:', !!(product.title && !product.draft));
                    }
                    ...
                    if (product.title && !product.draft) {
                        ...
                        if (filename === 'new-admin-product.md') {
                            console.log('=== NEW ADMIN PRODUCT SUCCESSFULLY ADDED ===');
                        }
                    } else {
                        console.log(`✗ Skipping ${filename}: ${!product.title ? 'no title' : 'is draft'}`);
                        
                        if (filename === 'new-admin-product.md') {
                            console.log('=== NEW ADMIN PRODUCT SKIPPED ===');
                            console.log('Reason: title missing =', !product.title, ', is draft =', product.draft);
                        }
                    }
                } else {
